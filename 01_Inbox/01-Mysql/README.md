...
- **[10-MySQL全场景数据恢复与灾难挽救实战方案.md](03-运维与管理/10-MySQL全场景数据恢复与灾难挽救实战方案.md)**：十轮深度推演、DML 行级闪回 (Flashback)、DDL 全备+PITR 任意点精准恢复、延时从库快速截断、磁盘碎片页雕刻及物理坏页阶梯抢救全生命周期恢复方案。
- **[11-MySQL-MHA高可用集群架构原理与生产落地实战指南.md](03-运维与管理/11-MySQL-MHA高可用集群架构原理与生产落地实战指南.md)**：MHA 经典高可用架构原理、强捞 Binlog 与差额日志补齐、VIP 漂移、Failover 状态机、生产部署全流程及避坑指南。
