...
### 3. Agent 上下文工程与持久化记忆治理 (Context Optimization & Memory)
*   **核心痛点**：多轮工具调用输出庞大（几十 KB 日志/网页/文件输出）迅速填满 Context Window，触发强制截断压缩导致 Agent 遗忘关键状态与任务。
*   **架构解法（参考开源热门 [mksglu/context-mode](https://github.com/mksglu/context-mode)）**：
    *   **沙箱执行与数据脱水**：大容量工具调用在沙箱内执行，仅向 Prompt 传递计算结果（实现 90%+ 上下文压缩）。
    *   **“Think in Code” 编程范式**：让 LLM 充当“代码生成器”而非“原始数据处理器”（让 Agent 写轻量脚本过滤数据，而非读取数十个全量文件）。
    *   **轻量检索式持久化记忆**：使用 SQLite FTS5 + BM25 增量索引会话事件与关键决策，会话重启或压缩后按需召回，避免全量上下文硬回填。
    *   **Prompt Cache (KV-Cache) 保护规范**：严格保证前缀稳定性，避免动态时间戳污染导致的缓存击穿。
